...
def calculate_total_expenses(expenses):
    total = sum(map(lambda x: x['jumlah'], expenses))
    return total
...
def get_expenses_by_date(expenses, date):
    ...
    for item in expenses:
        if(item['tanggal'] == date):
             list_expenses.append(item)  
             
    return list_expenses
             
# TODO 4 Buatlah fungsi generate_expenses_report disini
def generate_expenses_report(expenses):
    dates = sorted(set(item['tanggal'] for item in expenses))
    
    for date in dates:
        yield "Tanggal : " + date
        
        total = 0
        for item in get_expenses_by_date(expenses, date):
            yield item['deskripsi'] + "->" + str(item['jumlah'])
            total = total + item['jumlah']
            
        yield "Total Pengeluaran : " + str(total)
...
def add_expense_interactively(expenses):
   ...
   new_expenses = add_expense(expenses)
